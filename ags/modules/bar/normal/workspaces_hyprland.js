    ...
    return DrawingArea({
        ...
        setup: (area) => area
            .hook(Hyprland.active.workspace, (self) => {
                const offset = Math.floor((Hyprland.active.workspace.id - 1) / count) * userOptions.workspaces.shown;
                const activeLocal = Hyprland.active.workspace.id - offset;

                self.attribute.activeWs = activeLocal;
            ...
            .on('draw', Lang.bind(area, (area, cr) => {
                ...
                area.set_size_request(40, workspaceDiameter * count);

                const font = 'JetBrains Mono NF';
                const fontSize = '11';
                
                const widgetStyleContext = area.get_style_context();
                const activeWs = area.attribute.activeWs;

                const activeWsCenterX = width / 2;
                const activeWsCenterY = workspaceDiameter * (activeWs - 0.5);


                const layout = PangoCairo.create_layout(cr);
                const fontDesc = Pango.font_description_from_string(`${font} ${getFontWeightName(workspaceFontWeight)} ${fontSize}`);
                ...
                for (let i = 1; i <= count; i++) {
	                if (area.attribute.workspaceMask & (1 << i)) {
		                ...
		                const wsCenterY = (workspaceDiameter * i) - (workspaceDiameter / 2);
		                const wsCenterX = width / 2;
		
		                // Üst kısım: Eğer önceki workspace aktif değilse, yukarıdan yarım daire çiz.
		                if (!(area.attribute.workspaceMask & (1 << (i - 1)))) {
			                cr.arc(wsCenterX, wsCenterY , workspaceRadius, Math.PI, 2 * Math.PI);
			                cr.fill();
		                } else {
			                // Aktifse, üst kareyi çizerek bağlantı sağla.
			                cr.rectangle(wsCenterX - workspaceRadius, wsCenterY - workspaceRadius, workspaceRadius * 2, workspaceRadius);
			                cr.fill();
		                }
		
		                // Alt kısım: Eğer sonraki workspace aktif değilse, aşağıdan yarım daire çiz.
		                if (!(area.attribute.workspaceMask & (1 << (i + 1)))) {
			                cr.arc(wsCenterX, wsCenterY , workspaceRadius, 0, Math.PI);
			                cr.fill();
		                } else {
			                // Aktifse, alt kareyi çizerek bağlantı sağla.
...
export default () => EventBox({
    ...
    setup: (self) => {
        ...
        self.on('motion-notify-event', (self, event) => {
            ...
            if (cursorY > 275) return;
            const wsId = Math.ceil(cursorY * userOptions.workspaces.shown / 275);
