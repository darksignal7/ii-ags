...
import { searchItem , wallpaperSelect} from './searchitem.js';
...
export const StartupButton = (iconName = 'Error', name = 'Test', content = 'Test Content', command = '', ) => searchItem({
...
export const CustomCommandButton = ({ text = '', name = 'Action' }) => searchItem({
...
export const ClipHistoryButton = ({ text }) => searchItem({
    materialIconName: 'chat_paste_go',
    name: 'Clipboard',
    actionName: 'Copy',
    content: `${text}`,
    onActivate: () => {
        execAndClose('wl-copy ' + `${text}`);
    },
});

export const WallpaperButton = ({ icon, iconSize , realIcon }) => wallpaperSelect({
    icon: `${icon}`,
    iconSize: `${iconSize}`,
    realIcon: `${realIcon}`,
    onActivate: () => {
        //execAndNotClose('swww img ' + `${icon}`);
        //execAndClose('/home/vaguesyntax/.config/ags/scripts/color_generation/colorgen.sh' + `${icon}` + '--apply');
    },
});
