...
export function launchCustomCommand(command) {
    ...
    if (args[0] == '>raw') { // Mouse raw input
        ...
        Todo.add(args.slice(1).join(' '), args[1]);
    }
...
export function execAndNotClose(command, terminal) {
    if (terminal) {
        execAsync([`bash`, `-c`, `${userOptions.apps.terminal} fish -C "${command}"`, `&`]).catch(print);
    }
    else
...
export function execAndGet(command, terminal) {
    if (terminal) {
        return execAsync([`bash`, `-c`, `${userOptions.apps.terminal} fish -C "${command}"`, `&`]).catch(print);
    }
    else
        return execAsync(command).catch(print);
}
