...
import { DefaultPrompt } from '../../../assets/ai/Prompts.js';
...
export const sendMessage = (text) => {
    ...
    if (text.startsWith('/')) {
        if (text.startsWith('/clear')){
            ...
            clearChat();
            //chatContent.add(SystemMessage('Prompt Loaded!', '/prompt', GeminiView))
        }
            ...
            if (firstSpaceIndex == -1 || prompt.length < 1) {
                chatContent.add(SystemMessage(`Usage: \`/prompt FILEPATH\``, '/prompt', GeminiView))
            }
            else {
                //PromptPath = prompt;
