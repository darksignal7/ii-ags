...
import { TodoBar } from "./tools/todo.js";
...
const pinButton = Button({
    attribute: {
        ...
        'toggle': (self) => {
            ...
            const sideleftContent = sideleftWindow.get_children()[0].get_children()[0].get_children()[0].get_children()[0];

            sideleftContent.toggleClassName('sidebar-left-pinned', self.attribute.enabled);
            sideleftContent.toggleClassName('sidebar-left', !self.attribute.enabled);
...
const todosWidget = Widget.Scrollable({
    hscroll: "never",
    vscroll: "automatic",
    child: Box({
        vertical: true,
